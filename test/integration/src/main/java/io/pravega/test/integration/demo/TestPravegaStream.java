...
public class TestPravegaStream {
    ...
    public void testEventStreamCollector() {
        ...
        PravegaStream str = new PravegaStream(scope, stream, clientConfig);
        PravegaStream outputStream = new PravegaStream(scope, stream + "result", clientConfig);
        ...
        PravegaStream filteredStream = str.getUnorderedEventStream(serializer)
                ...
                .collect(new PravegaStreamCollector<>(outputStream, s -> s.split("-")[1], serializer, StreamConfiguration.builder().build()));

        // Verification
        assertEquals("Expected to read 100 events of type event-0 and event-1", 200L, filteredStream.getUnorderedEventStream(serializer).count());

        Map<String, Long> result = filteredStream.getUnorderedEventStream(serializer).collect(Collectors.groupingBy(
                Function.identity(), Collectors.counting()
        ));
    ...
    public void testOrderedEventStream() {
        ...
        PravegaStream str = new PravegaStream(scope, stream, clientConfig);
        // obtain a java.util.stream.Stream of bytes[]
        Stream<String> eventStream = str.getOrderedEventStream(new UTF8StringSerializer());
        //All aggregate options (map, flatMap, filter ...) supported by normal Java stream can be supported.
        Map<String, Long> result = eventStream
                .peek(s -> System.out.println(s)) // peek
                .collect(Collectors.groupingBy(
    ...
    public void testOrderedByteStream() {
        ...
        PravegaStream str = new PravegaStream(scope, stream, clientConfig);
        // read a ByteStream.
        Stream<byte[]> byteStream = str.getOrderedByteStream();
        //All aggregate options (map, flatMap, filter ...) supported by normal Java stream can be supported.
        Map<String, Long> result = byteStream.map(bytes -> StandardCharsets.UTF_8.decode(ByteBuffer.wrap(bytes)).toString()) // decode it.
                .peek(s -> System.out.println(s)) // peek
                .collect(Collectors.groupingBy(
    ...
    public void testOrderedEventStreamCollectorWithStreamScaling() throws Exception{
        ...
        PravegaStream filteredStream = str.getOrderedEventStream(serializer)
                ...
                .filter(s -> s.contains("event-0") || s.contains("event-1"))
                .collect(new PravegaStreamCollector<>(outputStream, s -> s.split("-")[1], serializer, StreamConfiguration.builder().build()));
        assertEquals("Expected to read 100 events of type event-0 and event-1", 200L, filteredStream.getOrderedEventStream(serializer).count());
    }
    ...
    private static void createAndWriteToStreamWithScale(URI controllerURI, ClientConfig clientConfig, io.pravega.client.stream.Stream stream) throws ExecutionException, InterruptedException {
        ...
        ControllerImpl controller = new ControllerImpl(ControllerImplConfig.builder()
                ...
                .maxBackoffMillis(5000).build(), controlerExecutor);
        // create scope and stream
        StreamManager streamManager = StreamManager.create(controllerURI);
        streamManager.createScope(stream.getScope());
        streamManager.createStream(stream.getScope(), stream.getStreamName(), StreamConfiguration.builder().scalingPolicy(ScalingPolicy.fixed(1)).build());

        EventStreamWriter<byte[]> writer = cf.createEventWriter(stream.getStreamName(), new ByteArraySerializer(), EventWriterConfig.builder().build());
        // write to segment 0
        for (int i=0; i < 100; i ++) {
            writer.writeEvent(keyReverseMap.get("0.1"), "event-0".getBytes(StandardCharsets.UTF_8));
        }
        ...
        Boolean scaleResult = controller.scaleStream(stream, Collections.singletonList(0L), map, controlerExecutor).getFuture().get();
